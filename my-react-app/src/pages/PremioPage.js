...
class PremiosPage extends React.Component {
    ...
    getData = () => {
        ...
        this.soapClient.invoke("verPremio", { id: match.params.id }).then(data => {
            this.setState({
                data: data,
                dataOk: true,
                title: "",
    ...
    render() {
        ...
        return (
            <GenericPage title={title} headerTitle={"Premios"}>
                {content}
            </GenericPage>
        );
    }
...
export default withRouter(withStyles(styles)(PremiosPage));