...
class EscuderiasPage extends React.Component {
    ...
    getData = () => {
        ...
        this.soapClient.invoke("verEscuderia", { id: match.params.id }).then(data => {
            this.setState({
                data: data,
                dataOk: true,
                title: "",
    ...
    render() {
        ...
        return (
            <GenericPage title={title} headerTitle={"Escuderías"}>
                {content}
            </GenericPage>
        );
    }
...
export default withRouter(withStyles(styles)(EscuderiasPage));