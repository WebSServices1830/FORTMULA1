...
class AutosPage extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            data: [],
    ...
    getPremios = () => {
        ...
        this.soapClient.invoke("verAutos").then(data => {
            this.setState({
                data: data,
                dataOk: true,
            });
    ...
    getDrawableGeneric = () => {
        ...
        if (this.state.dataOk) {
            ...
            for (const item of data) {
                ...
                const nombre = item["modelo"]["_text"];
                const description = item["motor"]["_text"];
    ...
    render() {
        return (
            <GenericPage headerTitle={"Autos"} title={"Autos"}>
...
export default withStyles(styles)(AutosPage);